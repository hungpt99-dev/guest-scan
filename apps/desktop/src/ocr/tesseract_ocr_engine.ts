...
export class TesseractOcrEngine implements OcrEngine {
  ...
  private async initialize(): Promise<void> {
    ...
    if (!isTauri()) {
      logger.debug("TesseractOcrEngine: not in Tauri context, marking unavailable");
      return;
    }

    try {
      const { invoke } = await import("@tauri-apps/api/tauri");
      this.available = await invoke<boolean>("check_tesseract_available");
    } catch (error) {
      logger.warn("TesseractOcrEngine: availability check failed", error);
      this.available = false;
    }
  }
  ...
  async extractText(input: OcrInput): Promise<OcrTextResult> {
    ...
    if (!this.available) {
      throw new Error("Tesseract OCR engine is not available");
    }
    ...
    }>("extract_tesseract_mrz", { imagePath: input.imagePath });
